...
def initialize_model(num_classes):
    ...
    model.heads[0] = nn.Linear(n_inputs, num_classes)  # Modify the last layer
    return model
...
def plot_learning_curves(train_acc, val_acc, train_loss, val_loss):
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.savefig('image_read.png')
    plt.show()

# Main function to run the setup
if __name__ == '__main__':
    data_dir = 'Image-Dataset'  # Update this path
    save_path = 'model'  # Update this path
    ...
    model, train_acc, val_acc, train_loss, val_loss = train_model(model, dataloaders, criterion, optimizer, save_path, num_epochs)
